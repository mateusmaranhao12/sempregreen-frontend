...
export default function EditarEsportePage({ params }: Props) {
    const { id } = use(params)
    const [showMenu, setShowMenu] = useState(true)

    const [nome, setNome] = useState('Bandy')
    ...
    return (
        <div className="flex flex-col md:flex-row gap-6 md:min-h-screen md:overflow-hidden">
            ...
            <main className="flex-1 px-4 py-4">
                ...
                {/* Cabeçalho superior com nome e botão de histórico */}
                <div className="flex justify-between items-center mb-4">
                    <div>
                        <h1 className="text-xl text-gray-700 font-semibold">Modificar {id} - Esporte</h1>
                        <p className="text-sm font-bold text-black">{nome} - {modalidade}</p>
                    </div>

                    <Link href={`/admin/esportes/${id}/editar/historico`}>
