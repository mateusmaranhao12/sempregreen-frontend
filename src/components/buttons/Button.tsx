...
interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
    variant?: 'yellow' | 'blue' | 'white' | 'green' | 'white_border_blue' | 'gray' | 'red' | 'darkgreen' | 'cyan'
    className?: string
}
...
export default function Button({ variant = 'yellow', className = '', ...props }: ButtonProps) {
  ...
  const variants = {
    ...
    darkgreen: 'bg-green-800 text-white cursor-pointer hover:bg-green-600',
    cyan: 'bg-cyan-500 text-white cursor-pointer hover:bg-cyan-900',
