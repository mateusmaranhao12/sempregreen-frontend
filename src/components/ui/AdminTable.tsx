                        ...
                        <tr key={rowIndex} className="hover:bg-gray-50">
                            ...
                            <td className="px-3 py-2 whitespace-nowrap flex gap-1">
                                {row.actions?.includes('add') && (
                                    <Link href={`/admin/${row.slug}/adicionar`} title="Adicionar" onClick={(e) => e.stopPropagation()}>
                                        <Button variant="green"><FaPlus size={16} /></Button>
                                    </Link>
                                )}
                                {row.actions?.includes('edit') && (
                                    <Link href={`/admin/${row.slug}/${row.id}/editar`} title="Editar" onClick={(e) => e.stopPropagation()}>
                                        <Button variant="blue"><FaEdit size={16} /></Button>
                                    </Link>
                                )}
                                {row.actions?.includes('remove') && (
                                    <Link href={`/admin/${row.slug}/remover`} title="Remover" onClick={(e) => e.stopPropagation()}>
                                        <Button variant="red"><FaTrash size={16} /></Button>
                                    </Link>
                                )}
                            </td>
